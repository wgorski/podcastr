...
void main() {
  group('Track', () {
    const sample = Track(
      ...
      subtitlePath: '/tmp/abc123.vtt',
      subtitleLanguage: 'en',
      subtitleIsAutoGenerated: true,
    ...
    test('copyWith clearFilePath nulls the file path', () {
      final cleared = sample.copyWith(clearFilePath: true);
      expect(cleared.filePath, isNull);
      // Other fields survive.
      expect(cleared.thumbnailPath, sample.thumbnailPath);
      expect(cleared.sourceUrl, sample.sourceUrl);
    });

    test('copyWith clearSubtitle nulls all subtitle fields', () {
      final cleared = sample.copyWith(clearSubtitle: true);
      expect(cleared.subtitlePath, isNull);
      expect(cleared.subtitleLanguage, isNull);
      expect(cleared.subtitleIsAutoGenerated, isFalse);
      // The thumbnail and audio path are untouched by clearSubtitle.
      expect(cleared.thumbnailPath, sample.thumbnailPath);
      expect(cleared.filePath, sample.filePath);
    });

    test('archiving keeps cover + metadata but drops audio/subtitle', () {
      final archived = sample.copyWith(
        archived: true,
        archivedAtMs: 123,
        clearFilePath: true,
        clearSubtitle: true,
      ...
      expect(archived.thumbnailPath, sample.thumbnailPath);
      expect(archived.sourceUrl, sample.sourceUrl);
      expect(archived.title, sample.title);
      expect(archived.archived, isTrue);
    });

    test('fromJson migrates a legacy row with no sourceUrl', () {
      final json = sample.toJson()..remove('sourceUrl');
      final restored = Track.fromJson(json);
      expect(restored.sourceUrl, 'https://youtu.be/abc123');
    });

    test('shareUrl returns the source url', () {
      expect(sample.shareUrl, 'https://youtu.be/abc123');
    });
